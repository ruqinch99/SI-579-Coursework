...
function showRhymes(result) {
    ...
    if (result.length != 0) {
        ...
        for (let numSyllables in groupOutput) {
            wordOutput.innerHTML += `<h3> Syllables: ${numSyllables}</h3>`;
            wordOutput.innerHTML += `<ul>`
            for (let word of groupOutput[numSyllables]) {
                wordOutput.innerHTML += `<li>${word['word']} <button type="button" class="btn btn-outline-success" onclick="addToSavedWords('${word['word']}')">(Save)</button></li>`;
            }
            wordOutput.innerHTML += `</ul>`;
        }
    } else {
...
function showSynonyms(result) {
    ...
    if (result != 0) {
        ...
        for (let word of result) {
            wordOutput.innerHTML += `<li>${word['word']} <button onclick="addToSavedWords('${word['word']}')"  type="button" class="btn btn-outline-success">(Save)</button></li>`;
        }
        wordOutput.innerHTML += `</ul>`;
    } else {
        wordOutput.innerHTML = `(no results)`;
